...
(function () {
  ...
  function ensureDrawer() {
    ...
    loadCss(ROOT + 'assets/css/account-drawer.css?v=29');
    _drawerLoading = (window.getSupabaseClient ? Promise.resolve() : loadScript(ROOT + 'assets/js/supabase-client.js?v=29'))
      .then(function () { return window.RA ? null : loadScript(ROOT + 'assets/js/auth.js?v=29'); })
      .then(function () { return loadScript(ROOT + 'assets/js/account-drawer.js?v=29'); });
  ...
  function openEventCalendarLazy(e) {
    if (e) e.preventDefault();
    ensureDrawer().then(function () { if (window.openEventCalendar) window.openEventCalendar(); })
      .catch(function (err) { console.error('event calendar load failed', err); window.location.href = ROOT + 'dashboard/'; });
  }

  // Custom RealLingo calendar mark: rounded page, yellow header band + rings,
  // event dots (two in RealLingo Yellow). Deliberately not a stock icon.
  var CAL_ICON_SVG =
    '<svg viewBox="0 0 24 24" width="21" height="21" fill="none" aria-hidden="true" focusable="false">' +
      '<rect x="3" y="4.5" width="18" height="16.5" rx="5" fill="#111216"/>' +
      '<rect x="3" y="4.5" width="18" height="5.5" rx="2.75" fill="#ffeb99"/>' +
      '<rect x="7" y="2" width="2.2" height="4.6" rx="1.1" fill="#111216"/>' +
      '<rect x="14.8" y="2" width="2.2" height="4.6" rx="1.1" fill="#111216"/>' +
      '<circle cx="8" cy="13.6" r="1.35" fill="#ffeb99"/>' +
      '<circle cx="12" cy="13.6" r="1.35" fill="#ffffff" opacity="0.45"/>' +
      '<circle cx="16" cy="13.6" r="1.35" fill="#ffffff" opacity="0.45"/>' +
      '<circle cx="8" cy="17.6" r="1.35" fill="#ffffff" opacity="0.45"/>' +
  ...
  function loadAuthStack() {
    return (window.getSupabaseClient ? Promise.resolve() : loadScript(ROOT + 'assets/js/supabase-client.js?v=29'))
      .then(function () { return window.RA ? null : loadScript(ROOT + 'assets/js/auth.js?v=29'); });
  }
  ...
  function applyAuthState(target) {
    ...
    // Logged in: "Hi, name" (name links to dashboard)  [cal] [cart]  [Log out]
    var name = '';
    try { name = localStorage.getItem('ra_name') || ''; } catch (e) {}
    var greet = name || (uname ? '@' + uname : 'there');
    actions.innerHTML =
      '<a href="' + ROOT + 'dashboard/" class="site-hi" id="siteUserBtn" title="Go to your dashboard">Hi, <b>' + escapeHtml(greet) + '</b></a>' +
    ...
    // Mobile: surface the Event Calendar icon in the header bar itself (right of
    // the logo) so it's reachable without opening the menu. Hidden on desktop
    // via CSS, where the calendar button in .site-actions is used instead.
  ...
  function loadPartial(name, targetId, after) {
    fetch(ROOT + 'partials/' + name + '?v=29')   // versioned so header/footer partials refresh with each release
